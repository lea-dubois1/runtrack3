...
class User
{
    ...
    public function Register($login, $password, $passwordConfirm, $email, $firstname, $lastname) {
        ...
        echo $error . $errorLogin . $errorPassword . $errorEmail . $errorNames;
    ...
    public function Connect($email, $password) {
        ...
        if($row == 1){    // If the email exist in the data base, continue
            ...
            if(password_verify($password,$dataPass)){    // Check if the password existe in the database and decript it
                ...
                $sucess = '<strong>Success!</strong> You\'re connected';

                header('Location: index.php');

            }else{    // If the password do not match, error
                $errorPass = '<strong>Error!</strong> Wrong password';
            }
        }else{    // If the login do not exist, error
            $errorEmail = '<strong>Error!</strong> The email is not unknow.';
        }

        $messages = [$sucess, $errorPass, $errorEmail];
